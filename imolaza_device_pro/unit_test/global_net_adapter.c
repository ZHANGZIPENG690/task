...
void m_impl_getip_event(void *event_data)
{
    ...
    if (event_data == NULL)
        return;
    _wifi.wifi_is_connected = true;
    DEBUG_TEST(DB_W,"Got IP (%d) .." , m_static_dec.status );
...
void m_net_global_adapter_set_dns(uint32_t main_dns, uint32_t backup_dns)
{
    ...
    DEBUG_TEST(DB_I,"Main Dns %d.%d.%d.%d  Back Dns %d.%d.%d.%d", m_static_dec.ip_info_gw[0],
            m_static_dec.ip_info_gw[1],
            m_static_dec.ip_info_gw[2], 1, 8, 8, 8, 8);
    // 备选
    memset(&dns_info, 0, sizeof(dns_info));
