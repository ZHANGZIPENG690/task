...
void QueueInit(QueRoot *qr)
{
    qr->headQueNode = qr->tailQueNode = NULL;
    anquan = false;
}

bool QueuePush(QueRoot *qroot, const char *que_data, int qn_len, uint64_t spec_cmd, uint64_t spec_cmd_flag, uint64_t timeout)
{
    // anquan = true;
    bool result = false;
...
bool QueuePop(QueRoot *qroot, char *out_que_data, uint64_t *spec_cmd, uint64_t *spec_cmd_flag, uint64_t *timec)
{
    bool re_data = false;
