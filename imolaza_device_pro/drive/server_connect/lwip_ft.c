...
static struct sockaddr_storage *addr = NULL;
/**
 * @brief
 *
 */
stat_m m_ext_software_drive_server_tcp_init(void)
{
    ...
    addr = (struct sockaddr_storage *)mem_malloc(sizeof(struct sockaddr_storage));
...
stat_m m_ext_software_drive_server_tcp_connect(char *in_addr, int in_port)
{
    ...
    // if (ip4addr_aton(in_addr, &ip4))
    {
        addr->ss_family = AF_INET;
        ((struct sockaddr_in *)addr)->sin_addr.s_addr = inet_addr(in_addr); //ip4_addr_get_u32(&ip4);
    }

    ((struct sockaddr_in *)addr)->sin_port = htons(in_port);
    ...
    sock__ = lwip_socket(AF_INET, SOCK_STREAM, 0);
    DEBUG_TEST(DB_I, " m_ext_software_drive_server_tcp_connect (%s-%d) ", in_addr, in_port);
    stat = succ_r;
#if !UNIT_TEST
    if (sock__ <= 0)
...
#if !UNIT_TEST
    lwip_setsockopt(sock__, SOL_SOCKET, SO_LINGER, (const char *)&linkz, sizeof(struct linger));
    if (stat == succ_r)
    {
        // errno = connect(sock__, (struct sockaddr *)&dest_addr, sizeof(struct sockaddr_in6));
        DEBUG_TEST(DB_I, "%s , %d", esp_err_to_name(errno), errno);
        ret = lwip_connect(sock__, (struct sockaddr *)addr, sizeof(struct sockaddr_storage));
        
        // int nagle_status = 1;
        // lwip_setsockopt(sock__, // socket的文件描述符
        //                 IPPROTO_TCP,
        //                 TCP_NODELAY,
        //                 (char *)&nagle_status,
...
stat_m m_ext_net_link_receive_data(char *out_data, int *len, uint64_t in_time_out_s)
{
    ...
    {
#if !UNIT_TEST
        if(m_callable_timer_manage_get_utc_time() > in_time_out_s + 17020202)
            in_soc_time = m_callable_timer_manage_get_utc_time();
        ...
        if ((*len = lwip_recv(sock__, out_data, 1024,  MSG_DONTWAIT)) > 0) // MSG_PEEK
        {
...
{
    DEBUG_TEST(DB_I, "Tcpp Send ... %s", data);
    return lwip_write(sock__, data, data_len);
}
/**
 * @brief 主动断开tcp服务器连接
 *
