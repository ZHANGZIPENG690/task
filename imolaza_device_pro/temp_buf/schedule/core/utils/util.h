 ...
 * @return false
 */
bool m_queue_pop_node(QueRoot *qRoot, char *outData, uint64_t *speccmd, uint64_t *cmd_flag, uint64_t *timec);
 ...
 */
bool m_queue_push_node(QueRoot *, const char *que_data, int qn_len, uint64_t spec_cmd, uint64_t spec_cmd_flag, uint64_t timeout);
/**
 * @brief Initialize nodes, each time each individual node queue needs to be initialized
 *
 * @param qRoot queue root node
 */
void m_queue_init(QueRoot *);
/**
 * @brief Delete a head node
 *
 * @param qRoot queue root node
 */
void m_queue_delete_node(QueRoot *);
