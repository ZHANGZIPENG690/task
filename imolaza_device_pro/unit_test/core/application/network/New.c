...
int m_static_instance_sack_algorithm_queue_gener(struct channel_config *arr, int max_channel, int water_start_time, struct instance_running_sequences *root_sequences, long in_tim_s)
{
    ...
    while (presentMaxChan < max_channel)
    {
        // if (arr[presentMaxChan].cd_channel_type != M_STATIC_CHANNEL_TYPE_NORMAL)
        {
            ...
            {
                if (arr[n].cd_channel_type == M_STATIC_CHANNEL_TYPE_NORMAL && arr[n].cd_should_run_time > arr[n].cd_reality_run_time)
                {
                    {
                        ...
                        // DEBUG_TEST( DB_I,"---> 区域 %d 浇水 %d 完成-%d    (%d , %d)   \r\n", arr[n].cd_cahnnel_id, arr[n].cd_sack_should_water_time,
                ...
                else if (arr[n].cd_should_run_time > 0 && arr[n].cd_should_run_time > arr[n].cd_reality_run_time)
                {
                    ...
                    if (arr[n].cd_running_end_time == 0 || temp_present_time - arr[n].cd_running_end_time >= arr[n].cd_sack_should_sack_time)
                    {
                            ...
                            arr[n].cd_reality_run_time + arr[n].cd_last_running_time < arr[n].cd_should_run_time)
                        {
                            ...
                            if (result == 0)
                            {
                                ...
                                if (dist < arr[n].cd_sack_should_water_time) // 如果现在的结果还没出来，但是开始时间的差距小于当前应该浇水的时间和，那么取差值，计算到精确的时间
                                {
                                    pc = n;
                                    result = 1; // 应该运行
                                ...
                                else if (dist >= (arr[n].cd_sack_should_water_time)) // 之间的差值大于当前的 ， 直接加上应该浇水的时间和
                                {
                                    arr[n].cd_reality_run_time += (arr[n].cd_sack_should_water_time);
                                    // DEBUG_TEST(DB_I,"(%hhd) watering : %d", waterObj->chaninfo[n].chanID, waterObj->chaninfo[n].realityRunTime);
                                    dist -= arr[n].cd_sack_should_water_time;
                                }
                            }
                        }
                        else
                        {
                            ...
                            // DEBUG_TEST( DB_I,".---> 区域 %d 浇水 %d 完成-%d    (%d , %d) \r\n", arr[n].cd_cahnnel_id, arr[n].cd_should_run_time - arr[n].cd_reality_run_time,
                            //        arr[n].cd_should_run_time, arr[n].cd_reality_run_time, final_water_sum);
                            {
                    ...
                    { // 要浸泡
                        if (arr[n].cd_should_run_time - arr[n].cd_reality_run_time > 0)
                        {
                            ...
                            // if (result == 0 && dist > 0 && (dist < sct)) // 如果现在的时间差 小于 浸泡时间
                            // {
                            //     pc = n;
                            //     result = 2; // 还应该浸泡多长时间
                            // }
...
int m_static_instance_algorithm_sack_time(uint8_t chann_id, float in_ar, float in_ir, uint32_t in_sack_should_run_time, uint8_t *ou_sack_time)
{
    ...
    if (in_ir == 0 && chann_id <= 6)
        in_ir = M_STATIC_FIXED_IR_FOR_CHANNEL[chann_id - 1];
    if (in_ar == 0 && chann_id <= 6)
        in_ir = M_STATIC_FIXED_AR_FOR_CHANNEL[chann_id - 1];
...
int m_static_instance_sack_algorithm_pro_max_A(struct channel_config *arr, int max_channel, long in_tim_s)
{
    ...
    if (!result)
    {
        ...
        for (int i = 0; i < max_channel; i++)
        {
            ...
            // 在这里生产区域的队列
            // struct instance_running_unit *new_running_unit = M_MALLOC(sizeof(struct instance_running_unit));
            // // 相同的序列有多少
            // new_running_unit->unit_similar_num = GET_CHANNEL_SHOULD_RUN_TIME(arr, i) / GET_CHANNEL_SACK_SHOULD_WATER_TIME(arr, i);
            // // 运行时间
            // new_running_unit->unit_should_running_time = GET_CHANNEL_SACK_SHOULD_WATER_TIME(arr, i);
            // // 运行区域
            // new_running_unit->unit_running_channel = GET_CHANNEL_ID(arr, i);
            // // 运行 状态
            // new_running_unit->state = M_STATIC_UNIT_STATUS_NEW;
            // p_root_array[GET_CHANNEL_ID(arr, i)][0] = new_running_unit;
            // if(arr[i].cd_channel_type == M_STATIC_CHANNEL_TYPE_WATERSAVING)
            if (arr[i].cd_channel_type != M_STATIC_CHANNEL_TYPE_NORMAL)
                ...
                DEBUG_TEST( DB_I,"区域%d ->  最佳浇水时长 %d  浸泡%d（不变）\n\n", GET_CHANNEL_ID(arr, i), GET_CHANNEL_SHOULD_RUN_TIME(arr, i));
        }
        // 高定
    }
    return 0;
...
int m_static_instance_sack_algorithm_pro_max(struct channel_config *arr, int max_channel, long in_tim_s)
{
    ...
    if (!result)
    {
        ...
        for (int i = 0; i < max_channel; i++)
        {
            ...
            int result_x = 0;
            //  填充单元序列
            if (arr[i].cd_should_run_time <= 0 || arr[i].cd_cahnnel_id <= 0 ||
                (arr[i].cd_channel_type == M_STATIC_CHANNEL_TYPE_SACK && (arr[i].cd_sack_should_water_time <= 0 || arr[i].cd_sack_should_sack_time <= 0))||
                (arr[i].cd_channel_type == M_STATIC_CHANNEL_TYPE_WATERSAVING && arr[i].cd_cahnnel_id  > 6 && (arr[i].cd_ar == 0 || arr[i].cd_asa == 0 || arr[i].cd_ir == 0))
                )
            {
            ...
            if (arr[i].cd_channel_type == M_STATIC_CHANNEL_TYPE_WATERSAVING)
            {
                m_static_instance_algorithm_water_time(arr[i].cd_cahnnel_id, arr[i].cd_asa, arr[i].cd_ar, arr[i].cd_ir, &arr[i].cd_sack_should_water_time);
                DEBUG_TEST( DB_I,"计算出区域ASA: %hhd \n", arr[i].cd_sack_should_water_time);
                ...
                for (int p = 1; p <= GET_CHANNEL_SHOULD_RUN_TIME(arr, i) - 3; p++) // GET_CHANNEL_SACK_SHOULD_WATER_TIME(arr , i)不会小于4
                {
                    ...
                    if (remainder >= FIX_TIME_MIN)
                    {
                        if (remainder + 0.99999 <= ((int)remainder) + 1 && remainder < GET_CHANNEL_SACK_SHOULD_WATER_TIME(arr, i)) // 如果整除了
                        {
                            ...
                            DEBUG_TEST( DB_I,"根据总浇水时长 %d   计算出最佳 (%d * %d)\n", arr[i].cd_should_run_time, (int)remainder, p + 1);
                                 ...
                                 remainder <= GET_CHANNEL_SACK_SHOULD_WATER_TIME(arr, i))
                        {
                            ...
                            DEBUG_TEST( DB_I,"根据总浇水时长 %d   计算出最佳 (%d * %d + %d)\n", arr[i].cd_should_run_time, (int)remainder, p, arr[i].cd_last_running_time);
                            result_x = (int)remainder;
                            break;
                        }
                    }
                    ...
                    // DEBUG_TEST( DB_I,"l%d ",GET_CHANNEL_SACK_SHOULD_WATER_TIME(arr, i));
                }
                // 有结果了 设置一下
                if (result_x != 0)
                    SET_CHANNEL_SACK_SHOULD_WATER_TIME(arr, i, result_x);
            ...
            if (arr[i].cd_channel_type != M_STATIC_CHANNEL_TYPE_NORMAL)
                ...
                DEBUG_TEST( DB_I,"区域%d ->  (无浸泡)最佳浇水时长 %d  浸泡%d（不变）\n\n", GET_CHANNEL_ID(arr, i), GET_CHANNEL_SHOULD_RUN_TIME(arr, i), 0);
        }
        // 高定
    }
    return result;
